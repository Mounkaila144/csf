...
class VendorService {
  ...
  async getCategories(page = 1, perPage = 10): Promise<PaginatedResponse<AdminCategory>> {
    const response = await fetch(
      `${API_BASE_URL}/vendor/categories?page=${page}&per_page=${perPage}`,
      {
        method: 'GET',
  ...
  async getCategory(id: number): Promise<ApiResponse<AdminCategory>> {
    const response = await fetch(`${API_BASE_URL}/vendor/categories/${id}`, {
      method: 'GET',
      headers: this.getHeaders(),
    });
  ...
  async getSubcategories(page = 1, perPage = 10, categoryId?: number): Promise<PaginatedResponse<AdminSubcategory>> {
    let url = `${API_BASE_URL}/vendor/subcategories?page=${page}&per_page=${perPage}`;
    if (categoryId) {
  ...
  async getSubcategory(id: number): Promise<ApiResponse<AdminSubcategory>> {
    const response = await fetch(`${API_BASE_URL}/vendor/subcategories/${id}`, {
      method: 'GET',
      headers: this.getHeaders(),
    });
  ...
  async createSubcategory(data: SubcategoryFormData): Promise<ApiResponse<AdminSubcategory>> {
    const response = await fetch(`${API_BASE_URL}/vendor/subcategories`, {
      method: 'POST',
      headers: this.getHeaders(),
      body: JSON.stringify(data),
    });
  ...
  async getProducts(page = 1, perPage = 10, filters?: {
    ...
    let url = `${API_BASE_URL}/vendor/products?page=${page}&per_page=${perPage}`;
